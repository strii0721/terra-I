...
class Default:
    ...
    def __init__(self):
        ...
        self.left_arm.construct(Link("link_0-0", np.array([5.1, 0, 0])))\
        ...
        self.hull = Hull()
        self.controller = SimulationController(self.left_arm,
                                               control_interval = self.CONTROL_INTERVAL).initialize()
        self.controller.bind_inverse_kinematic_analysis_basis(self.INVERSE_KINEMATIC_ANALYSIS_BASIS)
        self.renderer = Renderer(render_interval = self.RENDER_INTERVAL)
